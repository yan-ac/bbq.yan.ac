...
registerPage :: Maybe AccountId -> App Response
registerPage authResult = ok $ toResponse $ basic
  authResult
  "注册"
  ( do
      H.h1 $ do "注册友谊赛"
...
registerHandler :: Maybe AccountId -> App Response
registerHandler authResult = do
  ...
  if not (isValid (pack email))
    then badRequest $ toResponse $ resultTemplate "错误的邮箱地址格式"
    else if (length password) < 12 || (length password) > 24
      then badRequest $ toResponse $ resultTemplate "密码应当为 12—24 位"
      else do
        ...
        case isRegistered of
          True  -> forbidden $ toResponse $ resultTemplate "该邮箱已被注册"
          False -> do
...
finishRegistration :: Maybe AccountId -> App Response
finishRegistration authResult = do
  ...
  case result of
    Success _ -> do
      case decode . pack $ password of
        Left _          -> forbidden $ toResponse $ resultTemplate "错误的消息参数"
        Right packedPwd -> do
          let bcryptedPwd = unpack packedPwd
          creationResult <- update $ NewAccount (Email email, Password bcryptedPwd)
          case creationResult of
            Fail _    -> forbidden $ toResponse $ resultTemplate "注册失败，内部错误"
            Success _ -> do
              update $ DeleteEmailVCode (Email email)
              ok $ toResponse $ resultTemplate "注册成功"
    Fail "EXPIRED RECORD" -> unauthorized $ toResponse $ resultTemplate "验证信息已过期"
    Fail _                -> unauthorized $ toResponse $ resultTemplate "验证失败"

aboutRegister authResult runApp = do
  msum [
      dir "register" $ do
        nullDir
        method GET
        runApp (registerPage authResult)
    , dir "register" $ do
        nullDir
        method POST
        runApp (registerHandler authResult)
    , dir "finish-registration" $ runApp (finishRegistration authResult)
    ]
